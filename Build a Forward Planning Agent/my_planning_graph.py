...
class ActionLayer ( BaseActionLayer ) :
    ...
    def _inconsistent_effects ( self, actionA, actionB ) :
        ...
        #Check whether negated factor from either actionA or actionB are contained within set of effects of actionB or actionA, respectively.
        return any (  [ ~effect in actionA.effects for effect in actionB.effects ]  + \
                      [ ~effect in actionB.effects for effect in actionA.effects ]  ) 
    ...
    def _interference ( self, actionA, actionB ) :
        ...
        """
        # TODO: implement this function
        # Check whether any of negated preconditions of actionA or actionB  are contained within the set of preconditions of actionB or actionA, respectively.
        return any (  [ ~effect in actionA.preconditions for effect in actionB.effects ]  )  or \
               any (  [ ~effect in actionB.preconditions for effect in actionA.effects ]  ) 
    ...
    def _competing_needs ( self, actionA, actionB ) :
        ...
        """
        # TODO: implement this function
        # Check whether there is an existence of any mutexes between preconditionA and preconditionB from actionA and actionB respectively.
        return any ( self.parent_layer.is_mutex ( preconditionA, preconditionB )  for preconditionA in actionA.preconditions for preconditionB in actionB.preconditions )  or \
               any ( self.parent_layer.is_mutex ( preconditionB, preconditionA )  for preconditionA in actionA.preconditions for preconditionB in actionB.preconditions ) 
...
class LiteralLayer ( BaseLiteralLayer ) :
    ...
    def _inconsistent_support ( self, literalA, literalB ) :
        ...
        # Check if all ways to acheive both literals are pairwize mutex in the parent level.
        # Extract actions from both literals from parent, and check whether are those actions mutexe or not.
        return all ( self.parent_layer.is_mutex ( actionA, actionB )  for actionA in self.parents [ literalA ]  for actionB in self.parents [ literalB ]  )  and \
               all ( self.parent_layer.is_mutex ( actionB, actionA )  for actionA in self.parents [ literalA ]  for actionB in self.parents [ literalB ]  ) 

    def _negation ( self, literalA, literalB ) :
        """ Return True if two literals are negations of each other """
        # TODO: implement this function
        
        # Preety self-explanatory negation check part
...
class PlanningGraph:
    ...
    def h_levelsum ( self ) :
        ...
        # set initial levelsum to 0
        levelsum = 0
        
        # for every goal from goal list and every layer
        for goal in self.goal:
            for i, layer in enumerate ( self.literal_layers ) :
                # check whether goal is within particular layer
                if goal  in layer:
                    # if so, sum up the level sum with current layer index i
                    levelsum += i
                    break
    ...
    def h_maxlevel ( self ) :
        ...
        # set initial maxlevel to 0
        maxlevel = 0
        
        #for each goal in goals set iterate through set of layers
        for goal in self.goal:
            for i, layer in enumerate ( self.literal_layers ) :
                # if goal is within layer
                if goal  in layer:
                    # set maxlevel to maximum level index i reached so far
    ...
    def h_setlevel ( self ) :
        ...
        def allGoalsMet ( layer ):
            for goal in self.goal:
                # if goal is not in particular layer, then return false, otherwise, all goals are within the specified layer
                if goal not in layer:
                    return False
            return True

        # check whether goals are not mutual exclusve
        def goalsAreNoMutex ( layer ):
            # inspect all possible different goal pair combinations from goal set
            for goal1, goal2 in combinations( self.goal, 2 ):
                # if two different goals are mutex, then return false argument, otherwise, all specified goals are not mutex
                if layer.is_mutex( goal1, goal2 ):
                    return False 
            return True
 
        # initialize setlevel to 0
        setlevel = 0       
        
        # until the graph is not leveled, extend the graph
        while not self._is_leveled:     
            # fetch the last layer from literal layers
            last_layer = self.literal_layers[-1]  
            # if goals are met in last layer and no goals are mutex, then return level, otherwise, extend graph in order to satisfy criteria
